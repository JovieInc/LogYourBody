...
import { cloudinaryPublicIdForOwner } from "./asset-identity.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      console.error(
        "Missing Supabase service configuration for process-progress-photo",
      );
    ...
    // Validate inputs
    if (
      typeof storagePath !== "string" || typeof metricsId !== "string" ||
      !storagePath || !metricsId
    ) {
    ...
    if (metric.user_id !== user.id) {
      return jsonResponse({
        error: "Metrics record does not belong to the authenticated user",
      }, 403);
    }
    ...
    const originalUrl =
      `${supabaseUrl}/storage/v1/object/public/photos/${normalizedStoragePath}`;

    // Create Cloudinary upload URL
    const timestamp = Math.round(Date.now() / 1000);
    // Scope every destructive Cloudinary identifier to the authenticated
    // subject. Account deletion can then prove ownership without trusting a
    // client-selectable metric ID or mutable database URL.
    const publicId = cloudinaryPublicIdForOwner(user.id, metricsId, timestamp);
    ...
    const signature = hashArray.map((b) => b.toString(16).padStart(2, "0"))
      .join("");
    ...
    const processedUrl = uploadResult.eager?.[0]?.secure_url ||
