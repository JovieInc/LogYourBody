...
| `AUD-015` | Release observability config is redacted and trace sampling is validated            | `write_release_config.sh` records Sentry/Statsig configured booleans, rejects invalid `SENTRY_TRACES_SAMPLE_RATE`, and marks whether provider smoke proof is required                                   |
...
| `AUD-001` | App Store subscription-product verification now retries transient provider failures | `verify-app-store-subscriptions.rb` retries 5xx/429 and transient network failures; `verify-app-store-subscriptions-test.rb` covers retry success, bounded failure, and network retry                   |

## Latest Landed Release Evidence

| Evidence                | Current state                                                                                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Merged PR               | `#446` merged to `main` at `9b1171f61775aa2a826b72328310fb705f1a2571` on `2026-06-23T17:18:18Z`                                                                           |
| Post-merge CI           | GitHub run `28043804846` completed successfully                                                                                                                           |
| Web deploy              | GitHub Deploy run `28043804811` completed successfully                                                                                                                    |
| iOS release loop        | GitHub run `28043805529` failed on attempt 1 and failed again on attempt 2 at App Store Connect `subscriptionGroups` verification with Apple `UNEXPECTED_ERROR` HTTP 500  |
| TestFlight upload       | Skipped in run `28043805529` because the App Store Connect subscription verification failed before archive/export                                                         |
| GitHub release artifact | Prerelease `ios-v1.2.0-testflight.20260623102440` published at `2026-06-23T10:39:26Z`                                                                                     |
...
| App Store listing       | Public URL `https://apps.apple.com/us/app/logyourbody/id6755209876` returned HTTP `404` at Tue, 23 Jun 2026 10:39:46 GMT; this does not prove public listing availability |

## Guardrailed But Still Externally Blocked

| Issue     | Current state                                                                                                                                                                                                                                                                                                              | Required next proof                                                                                                                             |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUD-001` | TestFlight upload has prior workflow proof, and subscription-product verification now retries transient provider failures; latest release loop still hit Apple `UNEXPECTED_ERROR` 500 twice before archive/export; release docs still enforce `paywall_testflight_verified=false` until real purchase/restore proof exists | App Store Connect endpoint recovery, rerun release loop, account-owner TestFlight purchase and restore evidence plus App Store submission state |
| `AUD-004` | Local launch-quality and performance-unit gates now require non-empty XCTest evidence; post-merge CI run `28043804846` is the latest landed CI proof; frame/hitch claims remain blocked                                                                                                                                    | Reliable simulator metrics or physical-device Instruments/ETTrace artifact                                                                      |
| `AUD-015` | Release config now emits redacted Sentry/Statsig configured booleans, validates trace sample rate, and records whether provider smoke proof is required; provider smoke proof remains external                                                                                                                             | Production release summary plus smoke event/crash proof if enabled                                                                              |
...
- `pnpm --filter @logyourbody/shared-lib test -- release-observability-guard.test.ts`
- `ruby .github/scripts/verify-app-store-subscriptions-test.rb`
