        ...
        for batch in metrics.chunked(into: batchSize) {
            let metricsData = batch.map { metric -> [String: Any] in
                [
                    ...
                    "weight": metric.weight > 0 ? metric.weight : NSNull(),
                    "weight_unit": metric.weightUnit ?? "kg",
                    "waist_circumference": metric.waistCircumference > 0 ? metric.waistCircumference : NSNull(),
                    "hip_circumference": metric.hipCircumference > 0 ? metric.hipCircumference : NSNull(),
                    "waist_unit": metric.waistUnit ?? "cm",
                    "body_fat_percentage": metric.bodyFatPercentage > 0 ? metric.bodyFatPercentage : NSNull(),
                    "body_fat_method": metric.bodyFatMethod as Any? ?? NSNull(),
                    "muscle_mass": metric.muscleMass > 0 ? metric.muscleMass : NSNull(),
                    "bone_mass": metric.boneMass > 0 ? metric.boneMass : NSNull(),
                    "photo_url": metric.photoUrl as Any? ?? NSNull(),
                    "notes": metric.notes as Any? ?? NSNull(),
