...
**Boundary: compose/extend the existing token package.** Jovie's `design-system.css` and runtime font loader at revision `7641ffa76d03326542541c62080735c28190a1f0` define Inter for body/UI and Satoshi for editorial headings. The current `scripts/generate-design-tokens-export.mjs` exports colors only; private `@jovie/ui` is a separate application package LYB does not depend on. This narrow projection preserves the canonical roles and unchanged font bytes without importing a second application or linking sibling checkouts.
