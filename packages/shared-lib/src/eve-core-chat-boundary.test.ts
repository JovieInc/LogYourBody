...
describe('eve.dev core-chat boundary', () => {
  ...
  it('keeps production auth and product data unavailable in this slice', () => {
    expect(channelSource).toMatch(/auth:\s*\[localDev\(\), placeholderAuth\(\)\]/);
  ...
  it('contains no stale job-runner references in tracked files', () => {
    ...
    expect(Object.keys(packageJson.devDependencies ?? {})).not.toContain(`${dependencyScope}/build`);
  });
});
