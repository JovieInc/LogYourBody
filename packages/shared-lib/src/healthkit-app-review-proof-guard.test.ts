...
const releaseEvidence = readRepoFile('apps/ios/docs/development/RELEASE_EVIDENCE_PATH.md');
...
describe('AUD-017 HealthKit App Review proof guard', () => {
  ...
  it('requires allow, deny, and skip evidence before closing App Review proof', () => {
    const proofSource = releaseEvidence.toLowerCase();
    ...
    expect(proofSource).toContain('current `info.plist` usage strings');
    expect(proofSource).toContain('before claiming healthkit readiness');
  });
});
